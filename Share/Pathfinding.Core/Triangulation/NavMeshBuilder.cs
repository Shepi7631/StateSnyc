...
using NavPolygon = Pathfinding.Geometry.Polygon;
using NavVec2 = Pathfinding.Geometry.Vec2;
...
namespace Pathfinding.Triangulation
{
    public static class NavMeshBuilder
    {
        ...
        private static NavMesh2D Convert(TNetMesh tMesh)
        {
            ...
            var triangles = new List<Pathfinding.Geometry.NavTriangle>(tTris.Count);
            for (int i = 0; i < tTris.Count; i++)
            {
                var t = tTris[i];
